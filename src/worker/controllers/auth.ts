...
const config = (env: Env) => ({
	...
	navigationToken: env.NAVIGATION_TOKEN,
...
authRoutes.post('/totp', async (c) => {
	...
	if (!authService.isTotpConfigured(config(c.env))) {
	...
	if (!authService.validateTotp(body.code, c.env.TOTP_SECRET)) {
...
navigationAuthRoute.get('/', async (c) => {
	...
	if (
		!authService.isNavigationConfigured(config(c.env)) ||
		!authService.validateNavigationToken(c.req.query('token'), c.env.NAVIGATION_TOKEN)
	) {
		log('AuthController', '出参', c.get('requestId'), { error: 'INVALID_NAVIGATION_TOKEN' });
		throw unauthorized('Invalid navigation token');
	}
	await issueSession(c);
